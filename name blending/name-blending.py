__author__ = 'Nguyễn Quang Đức'
__version__ = '20190706'
__email__ = 'quangduc280497@gmail.com'
...
class DomainFuzz():
	...
	def __homoglyph(self):
		glyphs = {
		'a': [u'à', u'á', u'â', u'ã', u'ä', u'å', u'ɑ', u'ạ', u'ǎ', u'ă', u'ȧ', u'ą', 'q', 'ci', ],
		'b': ['6', 'd', 'lb', u'ʙ', u'ɓ', u'ḃ', u'ḅ', u'ḇ', u'ƅ'],
		'c': ['e', '(', u'ƈ', u'ċ', u'ć', u'ç', u'č', u'ĉ'],
		'd': ['b', 'cl', 'cl', 'dl', u'ɗ', u'đ', u'ď', u'ɖ', u'ḑ', u'ḋ', u'ḍ', u'ḏ', u'ḓ'],
		'e': ['c', u'é', u'è', u'ê', u'ë', u'ē', u'ĕ', u'ě', u'ė', u'ẹ', u'ę', u'ȩ', u'ɇ', u'ḛ'],
		'f': [u'ƒ', u'ḟ'],
		'g': ['q', '9', 'cj', u'ɢ', u'ɡ', u'ġ', u'ğ', u'ǵ', u'ģ', u'ĝ', u'ǧ', u'ǥ'],
	...
	def run(self):
		while not self.kill_received:
			...
			if self.option_whois:
				if nxdomain is False and 'dns-ns' in domain:
					try:
						whoisdb = whois.whois(domain['domain-name'])
						domain['whois-registrar'] = str(whoisdb.registrar).split(' ')[0]
						domain['whois-whois_server'] = str(whoisdb.whois_server).split(' ')[0]
						from datetime import datetime
						if str(type(whoisdb.updated_date)) == "<class 'list'>" or str(type(whoisdb.creation_date)) == "<class 'list'>":
							domain['whois-created'] = str(whoisdb.creation_date[0]).split(' ')[0]
							domain['whois-updated'] = str(whoisdb.updated_date[0]).split(' ')[0]
							domain['whois-lived'] = datetime(year = int(str(str(whoisdb.updated_date[0]).split(' ')[0]).split('-')[0]), month = int(str(str(whoisdb.updated_date[0]).split(' ')[0]).split('-')[1]), day = int(str(str(whoisdb.updated_date[0]).split(' ')[0]).split('-')[2]), hour = int(str(str(whoisdb.updated_date[0]).split(' ')[1]).split(':')[0]), minute = int(str(str(whoisdb.updated_date[0]).split(' ')[1]).split(':')[1]), second = int(str(str(whoisdb.updated_date[0]).split(' ')[1]).split(':')[2])) - datetime(year = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[0]), month = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[1]), day = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[2]), hour = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[0]), minute = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[1]), second = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[2]))
							domain['whois-emails'] = str(whoisdb.emails).split(' ')[0]
							domain['whois-name'] = str(whoisdb.name).split(' ')[0]
							domain['whois-address'] = str(whoisdb.address).split(' ')[0]
							domain['whois-city'] = str(whoisdb.city).split(' ')[0]
							domain['whois-state'] = str(whoisdb.state).split(' ')[0]
							domain['whois-country'] = str(whoisdb.country).split(' ')[0]
						else:
							domain['whois-created'] = str(whoisdb.creation_date).split(' ')[0]
							domain['whois-updated'] = str(whoisdb.updated_date).split(' ')[0]
							domain['whois-lived'] = datetime(year = int(str(str(whoisdb.updated_date).split(' ')[0]).split('-')[0]), month = int(str(str(whoisdb.updated_date).split(' ')[0]).split('-')[1]), day = int(str(str(whoisdb.updated_date).split(' ')[0]).split('-')[2]), hour = int(str(str(whoisdb.updated_date).split(' ')[1]).split(':')[0]), minute = int(str(str(whoisdb.updated_date).split(' ')[1]).split(':')[1]), second = int(str(str(whoisdb.updated_date).split(' ')[1]).split(':')[2])) - datetime(year = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[0]), month = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[1]), day = int(str(str(whoisdb.creation_date).split(' ')[0]).split('-')[2]), hour = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[0]), minute = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[1]), second = int(str(str(whoisdb.creation_date).split(' ')[1]).split(':')[2]))
							domain['whois-emails'] = str(whoisdb.emails).split(' ')[0]
							domain['whois-name'] = str(whoisdb.name).split(' ')[0]
							domain['whois-address'] = str(whoisdb.address).split(' ')[0]
							domain['whois-city'] = str(whoisdb.city).split(' ')[0]
		...
		if 'whois-created' in domain and 'whois-updated' in domain:
			if domain['whois-created'] == domain['whois-updated']:
				if 'whois-registrar' in domain:
					info += '%sRegistrar:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-registrar'], FG_RST)
				if 'whois-whois_server' in domain:
					info += '%sServer:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-whois_server'], FG_RST)
				info += '%sCreated/Updated:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-created'], FG_RST)
				if 'whois-emails' in domain:
					info += '%sEmails:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-emails'], FG_RST)
				if 'whois-name' in domain:
					info += '%sName:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-name'], FG_RST)
				if 'whois-address' in domain:
					info += '%sAddress:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-address'], FG_RST)
				if 'whois-city' in domain:
					info += '%sCity:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-city'], FG_RST)
				if 'whois-state' in domain:
					info += '%sState:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-state'], FG_RST)
				if 'whois-country' in domain:
					info += '%sCountry:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-country'], FG_RST)
			else:
				if 'whois-registrar' in domain:
					info += '%sRegistrar:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-registrar'], FG_RST)
				if 'whois-whois_server' in domain:
				...
				if 'whois-lived' in domain:
					info += '%sLived:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-lived'], FG_RST)
				if 'whois-emails' in domain:
					info += '%sEmails:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-emails'], FG_RST)
				if 'whois-name' in domain:
					info += '%sName:%s%s%s ' % (FG_YEL, FG_CYA, domain['whois-name'], FG_RST)
...
''' 
 _   _                        ____  _                _ _
| \ | | __ _ _ __ ___   ___  | __ )| | ___ _ __   __| (_)_ __   __ _
|  \| |/ _` | '_ ` _ \ / _ \ |  _ \| |/ _ \ '_ \ / _` | | '_ \ / _` |
| |\  | (_| | | | | | |  __/ | |_) | |  __/ | | | (_| | | | | | (_| |
|_| \_|\__,_|_| |_| |_|\___| |____/|_|\___|_| |_|\__,_|_|_| |_|\__, |
                                                               |___/ {%s}
